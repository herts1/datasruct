...
static int hash_div_name(const char *name, int size) {
    ...
    return (int)(v % (unsigned long)size);     //将字母转化为无符号数字再计算，最后int返回 
}
...
static int hash_phone(const char *phone, int size, int htype) {
    return htype == HASH_DIV ? hash_div_phone(phone, size): hash_fold_phone(phone, size);
}
static int hash_name(const char *name, int size, int htype) {
    return htype == HASH_DIV ? hash_div_name(name, size): hash_fold_name(name, size);
}
...
void count_calls(void) {
    ...
    for (int k = 0; k < call_cnt; k++) {
        for (int i = 0; i < cust_cnt; i++) {
            if (strcmp(customers[i].phone, call_from[k]) == 0) {
				customers[i].call_count++;
				} 
            if (strcmp(customers[i].phone, call_to[k])   == 0) {
				customers[i].call_count++;
				} 
        }
    }
...
void compare_hash_functions(void) {
    ...
    printf("[手机号哈希表 - 开放寻址]\n");
    printf("  %-10s  冲突数=%d  冲突率=%.2f%%\n","除留余数", s_div.collisions,100.0 * s_div.collisions / s_div.inserted);
    printf("  %-10s  冲突数=%d  冲突率=%.2f%%\n\n","折叠法", s_fold.collisions,100.0 * s_fold.collisions / s_fold.inserted);
    ...
    printf("[姓名哈希表 - 开放寻址]\n");
    printf("  %-10s  冲突数=%d  冲突率=%.2f%%\n","除留余数", sn_div.collisions,100.0 * sn_div.collisions / sn_div.inserted);
    printf("  %-10s  冲突数=%d  冲突率=%.2f%%\n\n","折叠法", sn_fold.collisions,100.0 * sn_fold.collisions / sn_fold.inserted);
}
